...
INT_PTR CALLBACK VentanaConfiguracion(HWND hwndConf, UINT mensaje, WPARAM wParam, LPARAM lParam) {
	...
	switch (mensaje)
	{
		case WM_INITDIALOG:
			...
			CheckDlgButton(hwndConf, ID_A_RADIO, radioInteligente.obtenerValorAviso(A_RADIO));
		...
		case WM_COMMAND:
		{
			switch (LOWORD(wParam))
			{
				...
				case IDOK:
					...
					radioInteligente.establecerValorAviso(A_RADIO, IsDlgButtonChecked(hwndConf, ID_A_RADIO));
